...
이 애너테이션에 적용할 수 있는 대상을 '@Target'으로 지정하였다.  
`@Target({TYPE, FIELD, METHOD, PARAMETER, CONSTRUCTOR, LOCAL_VARIABLE})`  

<br>

#### @Retention  
애너테이션이 유지되는 기간을 징정하는데 사용된다. 
...
유지 정책은 'RUNTIME'으로 하면, 실행 시에 reclection을 통해 클래스 파일에 저장된 애너테이션의 정보를 읽어서 처리할 수 있다.  
'@FuctionalInterface'는 '@Override'처럼 컴파일러가 체크해주는 애너테이션이지만, 
실행 시에도 사용되므로 유지정책이 'RUNTIME'으로 되어 있다.  

유지 정책 'CLASS는 컴파일러가 애너테이션의 정보를 클래스 파일에 저장할 수 있게는 하지만, 클래스 파일이 JVM에 로딩될 때는 애너테이션의 정보가 무시되어 실행 시에 애너테이션에 대한 정보를 얻을 수 없다. 
...
애너테이션의 요소는 **반환값이 있고 매개변수는 없는 추상 메서드의 형태**를 가지며, 상속을 통해 구현하지 않아도 된다.  
...
예를 들어, '@SuppressWarnings'의 경우, 요소의 타입이 String 배열이고 이름이 value이다.  
...
게다가 Annotation은 애너테이션이 아니라 **일반적인 인터페이스**로 정의되어 있다.  

모든 애너테이션의 조상인 Annotation 인터페이스가 정의되어 있기 때문에, 모든 애너테이션 객체에 대해 equals(), hashCode(), toString()과 같은 메서드를 호출하는 것이 가능하다.  
